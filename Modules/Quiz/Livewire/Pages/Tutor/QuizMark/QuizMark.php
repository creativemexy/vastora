...
class QuizMark extends Component
{
    ...
    public function mount(int $attemptId = null)
    {
        ...
        if (empty($attemptQuiz) || $attemptQuiz->result != QuizAttempt::RESULT_IN_REVIEW) {
            abort(404);
        }
    ...
    {
        if (empty($this->requiredAnswers)) {
            $this->dispatch('showToast', type: 'error', message: __('quiz::quiz.no_answers_found'));
            return;
        }
