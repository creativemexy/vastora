...
class EditQuiz extends Component
{
    ...
    public function mount($quizId = null)
    {
        ...
        if($this->form->quizzable_type == UserSubjectGroupSubject::class){
            $slotData = $this->bookingService->getAvailableSubjectSlots($this->form->quizzable_id, $this->dateFormat, $this->timeFormat, $this->quiz?->status);
    ...
    public function updatedForm($value, $key)
    {
        if (str_contains($key, 'quizzable_type')) {
            ...
            if ($this->form->quizzable_type == UserSubjectGroupSubject::class) {
                $this->slots = $this->bookingService->getAvailableSubjectSlots($this->form->quizzable_id, $this->dateFormat, $this->timeFormat, $this->quiz?->status);
    ...
    public function updateQuiz($continue = false)
    {
        $response = isDemoSite();
        if ($response) {
        ...
        if($continue){
            return redirect()->route('quiz.tutor.quiz-settings', ['quizId' => $this->quizId]);
        }
    }
