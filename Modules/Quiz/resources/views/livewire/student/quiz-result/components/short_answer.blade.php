...
<div class="am-question {{!$attemptedQuestions?->is_correct ? 'am-wrong-answer' :  '' }}">
    <div class="am-question_title">
        ...
        @elserole('tutor')
            <div class="am-question_status am-checkbox">
                ...
                <label>/ {{ $attemptedQuestions?->question->points }}
                    Answer is correct <input type="checkbox" wire:model="requiredAnswers.{{ $question_index }}.is_correct" value="1" name="{{ 'answer_'.$loop->index + 1  }}"/>
                </label>
            </div>
        @endrole
    </div>
