...
class QuizService
{
        ...
        return Quiz::query()
            ...
            ->with([
                'settings' => function ($query) {
                    $query->where('meta_key', 'duration')->limit(1);
                }
            ])
     ...
     * @return \Illuminate\Contracts\Pagination\LengthAwarePaginator
     */
    public function getAttemptedQuizzes(?array $select = ['*'], ?array $relations = [], ?int $tutorId = null, array $filters = [], $quizId = null, int $studentId = null, array $withCount = [], $role = null)
    {
    ...
    {
        return empty($relations) ? QuizAttempt::find($attemptId) : QuizAttempt::with($relations)->find($attemptId);
    }
     ...
     * @return \Illuminate\Database\Eloquent\Builder
     */
    protected function buildAttemptedQuizQuery(?array $select, ?array $relations = [], ?int $tutorId = null, array $filters = [], $quizId = null, int $studentId = null, array $withCount = [], $role = null)
    {
        return QuizAttempt::query()
            ...
            ->when(!empty($filters['keyword']), function ($query) use ($filters) {
                return $query->whereHas('quiz', function ($subQuery) use ($filters) {
                    $subQuery->where('title', 'like', "%{$filters['keyword']}%");
                });
            })
     ...
     * @throws \Illuminate\Database\Eloquent\ModelNotFoundException
     */
    public function addQuizSettings(int $quizId, array $settings): void
    {
    ...
    {
        // Get the original quiz with its relationships
        ...
        // Create a new quiz with copied attributes
        $newQuiz = $originalQuiz->replicate();
        $newQuiz->title = $newQuiz->title . ' (Copy)';
        $newQuiz->status = Quiz::STATUS_DRAFT;
        $newQuiz->save();
        ...
        foreach ($originalQuiz->questions as $question) {
            ...
            // Duplicate question media if exists
            if ($question->media->count() > 0) {
                foreach ($question->media as $media) {
                    $newMedia = $media->replicate();
                    $newMedia->mediable_id = $newQuestion->id;
                    $newMedia->save();
                }
            }
